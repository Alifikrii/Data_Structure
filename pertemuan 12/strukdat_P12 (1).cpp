	...
	while (1)
	{
		...
		case 2:
			if (root == NULL) { 
				...
				continue;}
			cout<<"Ketik angka yang mau dihapus dari BST: ";
			cin>>num; 
			bst.fun_2(num); 
			break;
		case 3:
			temp = new node;
			cout<<"Ketik angka yang mau dimasukkan ke BST: ";
			cin>>temp->info; 
			bst.fun_3(root, temp); 
			break;
		case 4:
...
void BST::fun_1(int item, node **par, node **loc) //3
{
	node *ptr, *ptrsave;
	if (root == NULL) {
		*loc = NULL;
...
void BST::fun_3(node *tree, node *newnode) //4
{
	if (root == NULL)
	{
		root = new node;
...
void BST::case_a(node *par, node *loc ) //6
{
	if (par == NULL)
	{
		root = NULL;
...
void BST::case_b(node *par, node *loc) //7
{
	node *child;
	if (loc->left != NULL)
		child = loc->left;
...
void BST::case_c(node *par, node *loc) //8
{
...
void BST::fun_4(node *ptr, int level) //9
{
	int i;
	if (ptr != NULL)
	{
